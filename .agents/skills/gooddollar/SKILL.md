...
description: >
  ...
  stream, identity, and AntSeed AI credits (buy with G$, buyer CLI/API setup) tasks.
  Prefer GoodDocs (https://docs.gooddollar.org/) for narrative; contract addresses
  only from GoodProtocol releases/deployment.json (AntSeed integration addresses
  are documented in the AntSeed guides, not deployment.json).
metadata:
  version: 1.0.0
license: MIT
---
...
- `references/guides/antseed-ai-credits.md` — buy AntSeed AI credits with G$ (widget + Celo vault + Worker funding).
- `references/guides/antseed-buyer-setup.md` — AntSeed buyer key, CLI proxy, peer pin, and local API setup.
...
- Buy AntSeed AI credits with G$, credit status, or funding bridge -> `references/guides/antseed-ai-credits.md`
- AntSeed buyer key, `ANTSEED_IDENTITY_HEX`, local proxy, or AI tool API setup -> `references/guides/antseed-buyer-setup.md`
...
- AntSeed AI credits: confirm Worker `/config/status` bridge enabled; vault calldata/`userData` encodes buyer; distinguish payer (G$ on Celo) from buyer (Base deposit / `ANTSEED_IDENTITY_HEX`).
