...
import worker, { accountFromRequest } from "../src/worker.js";
...
test("account selector can come from body, x-gooddollar-account, Authorization, or x-api-key", () => {
  const wallet = "0x0000000000000000000000000000000000000abc";
  assert.equal(accountFromRequest(new Request("https://worker.test/v1/chat/completions"), wallet), wallet);
  assert.equal(accountFromRequest(new Request("https://worker.test/v1/chat/completions", { headers: { "x-gooddollar-account": wallet } })), wallet);
  assert.equal(accountFromRequest(new Request("https://worker.test/v1/chat/completions", { headers: { authorization: `Bearer gd:${wallet}` } })), wallet);
  assert.equal(accountFromRequest(new Request("https://worker.test/v1/chat/completions", { headers: { "x-api-key": `account:${wallet}` } })), wallet);
});

test("config status documents the GoodDollar backend proxy flow", async () => {
  const res = await worker.fetch(new Request("https://worker.test/config/status"), env(), {} as ExecutionContext);
  assert.equal(res.status, 200);
  const body = await res.json() as {
    openAiCompatible: { chatCompletionsPath: string; accountSelectors: string[] };
    antseed: { model: string; baseUrlConfigured: boolean };
  };
  assert.equal(body.openAiCompatible.chatCompletionsPath, "/v1/chat/completions");
  assert.equal(body.openAiCompatible.accountSelectors.includes("Authorization: Bearer gd:0x..."), true);
  ...
  try {
    const chatRes = await worker.fetch(new Request("https://worker.test/v1/chat/completions", {
      ...
      headers: {
        ...
        authorization: `Bearer gd:${wallet}`
      },
      body: JSON.stringify({
    ...
    const profileRes = await worker.fetch(new Request(`https://worker.test/v1/accounts/${wallet}/credit`), testEnv, {} as ExecutionContext);
