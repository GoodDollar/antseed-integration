...
contract MockGdToken is IERC20Like {
    ...
    function erc777Send(address operator, address from, address to, uint256 amount, bytes calldata userData) external returns (bool) {
        ...
        balanceOf[to] += amount;
        (bool ok, ) = to.call(
            abi.encodeWithSignature("tokensReceived(address,address,address,uint256,bytes,bytes)", operator, from, to, amount, userData, bytes(""))
        );
    ...
    function callTokensReceivedWithTo(address target, address from, address to, uint256 amount, bytes calldata userData) external returns (bool) {
        (bool ok, ) = target.call(
            abi.encodeWithSignature("tokensReceived(address,address,address,uint256,bytes,bytes)", address(this), from, to, amount, userData, bytes(""))
        );
        return ok;
    }
}
...
contract MockSuperfluidHost {
    ...
    function updateFlow(CeloGdAntSeedVault vault, address superToken, address sender, int96 flowRate, bytes calldata ctx) external returns (bytes memory) {
    ...
    function terminateFlow(CeloGdAntSeedVault vault, address superToken, address sender, bytes calldata ctx) external returns (bytes memory) {
...
contract MockRevertingOracle {
    function quoteAllAvailablePoolsWithTimePeriod(uint128, address, address, uint32) external pure returns (uint256, address[] memory) {
...
contract Outsider {
    ...
    constructor(CeloGdAntSeedVault vault_) {
        vault = vault_;
    }
    ...
    function setSuperfluidConfig(address h, address c) external {
        vault.setSuperfluidConfig(h, c);
    }

    function setStaticOracleConfig(address o, address c, uint256 f) external {
        vault.setStaticOracleConfig(o, c, f);
    }

    function setMinimumUsdThresholds(uint256 a, uint256 b) external {
        vault.setMinimumUsdThresholds(a, b);
    }

    function registerSuperApp(uint256 w) external {
        vault.registerSuperApp(w);
    }
}
...
contract CeloGdAntSeedVaultTest {
    ...
    function setUp() public {
        ...
        ERC1967Proxy proxy = new ERC1967Proxy(address(impl), abi.encodeCall(CeloGdAntSeedVault.initialize, (address(this), address(host), address(cfa))));
    ...
    function testSuperAppRejectsWrongTokenAgreementReceiverAndNegativeFlow() public {
        ...
        (bool ok, ) = address(host).call(
            abi.encodeWithSignature("createFlow(address,address,address,int96,bytes)", address(vault), address(wrongToken), address(user), int96(1), "")
        );
        require(!ok, "wrong token rejected");

        MockCFA otherCfa = new MockCFA();
        vault.setSuperfluidConfig(address(host), address(otherCfa));
        (ok, ) = address(host).call(
            abi.encodeWithSignature("createFlow(address,address,address,int96,bytes)", address(vault), address(token), address(user), int96(1), "")
        );
    ...
    function testUsesStaticOracleForStreamMinimum() public {
        ...
        (bool ok, ) = address(host).call(
            abi.encodeWithSignature("createFlow(address,address,address,int96,bytes)", address(vault), address(token), address(user), lowFlow, "")
        );
    ...
    function testCannotReinitialize() public {
        setUp();
        (bool ok, ) = address(vault).call(abi.encodeCall(CeloGdAntSeedVault.initialize, (address(this), address(host), address(cfa))));
    ...
    function testOnlyOwnerCanUpgrade() public {
        ...
        (bool ok, ) = address(outsider).call(abi.encodeWithSignature("upgrade(address,address)", address(vault), address(newImpl)));
    ...
    function testDepositRejectsZeroAmount() public {
        setUp();
        (bool ok, ) = address(vault).call(abi.encodeWithSignature("deposit(uint256,bytes)", uint256(0), abi.encode(BUYER)));
    ...
    function testDepositRejectsMissingBuyer() public {
        setUp();
        user.approveVault(1 ether);
        (bool ok, ) = address(vault).call(abi.encodeWithSignature("deposit(uint256,bytes)", uint256(1 ether), bytes("")));
    ...
    function testErc677RejectsMissingBuyer() public {
        ...
        (bool ok, ) = address(token).call(abi.encodeWithSignature("transferAndCall(address,uint256,bytes)", address(vault), uint256(1 ether), bytes("")));
        require(!ok, "erc677 missing buyer rejected");
    }

    // --- UnsupportedToken: onlyGdToken modifier ---

    function testOnTokenTransferRejectsNonGdToken() public {
        setUp();
        (bool ok, ) = address(vault).call(
    ...
    function testTransferFailedRevertsDeposit() public {
        ...
        (bool ok, ) = address(failVault).call(abi.encodeWithSignature("deposit(uint256,bytes)", uint256(1 ether), abi.encode(BUYER)));
    ...
    function testSetStaticOracleConfigRejectsInvalidPriceConfig() public {
        setUp();
        (bool ok, ) = address(vault).call(abi.encodeWithSignature("setStaticOracleConfig(address,address,uint256)", address(0), address(0), uint256(0)));
    ...
    function testOnlyOwnerFunctionsRejectNonOwner() public {
        ...
        (bool ok, ) = address(outsider).call(abi.encodeWithSignature("transferOwnership(address)", address(0xDEAD)));
        require(!ok, "non-owner transferOwnership rejected");

        (ok, ) = address(outsider).call(abi.encodeWithSignature("setSuperfluidConfig(address,address)", address(0), address(0)));
        require(!ok, "non-owner setSuperfluidConfig rejected");

        (ok, ) = address(outsider).call(abi.encodeWithSignature("setStaticOracleConfig(address,address,uint256)", address(0xAB), address(0xAB), uint256(0)));
        require(!ok, "non-owner setStaticOracleConfig rejected");

        (ok, ) = address(outsider).call(abi.encodeWithSignature("setMinimumUsdThresholds(uint256,uint256)", uint256(1), uint256(1)));
    ...
    function testTransferOwnershipRejectsZeroAddress() public {
        setUp();
        (bool ok, ) = address(vault).call(abi.encodeWithSignature("transferOwnership(address)", address(0)));
    ...
    function testTransferOwnershipWorks() public {
        ...
        (bool ok, ) = address(vault).call(abi.encodeWithSignature("setSuperfluidConfig(address,address)", address(host), address(cfa)));
    ...
    function testRegisterSuperAppRejectsZeroHost() public {
        ...
        (bool ok, ) = address(vault).call(abi.encodeWithSignature("registerSuperApp(uint256)", uint256(12345)));
    ...
    // --- depositFrom via Superfluid batch ---

    /// @dev Superfluid batch calls invoke `depositFrom(sender, amount, data)` on behalf of the user.
    ///      The vault must record the deposit under `sender` and pull tokens from `sender`, not msg.sender (the host).
    function testDepositFromViaSuperfluidBatch() public {
        setUp();
        // Pre-approve vault to pull tokens from `user`.
        user.approveVault(75 ether);

        // Route through MockSuperfluidHost so msg.sender == host (onlySuperfluidHost passes).
        (bool ok, bytes memory retData) = address(host).call(
            abi.encodeWithSignature("callDepositFrom(address,address,uint256,bytes)", address(vault), address(user), uint256(75 ether), abi.encode(BUYER))
        );
        require(ok, "depositFrom via host succeeded");
        uint256 total = abi.decode(retData, (uint256));

        require(total == 75 ether, "returned total matches");
        require(vault.totalDepositedGd(address(user)) == 75 ether, "deposit recorded under sender");
        require(token.balanceOf(address(vault)) == 75 ether, "vault holds transferred tokens");
    }

    function testDepositFromRejectsNonHost() public {
        setUp();
