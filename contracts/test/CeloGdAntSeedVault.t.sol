...
contract MockGdToken is IERC20Like {
    ...
    function callTokensReceivedWithTo(
        address target,
        address from,
        address to,
        uint256 amount,
...
contract MockCFA {
    ...
    function getFlow(
        ...
        address sender,
        address
    ) external view returns (uint256, int96, uint256, uint256) {
...
contract MockSuperfluidHost {
    ...
    function decodeCtx(
        bytes calldata ctx
    ) external pure returns (ISuperfluidHostLike.Context memory context) {
    ...
    function createFlow(
        ...
        bytes memory cbdata = vault.beforeAgreementCreated(
            superToken,
            address(cfa),
            bytes32(0),
            abi.encode(sender, address(vault)),
            ctx
        );
        cfa.setFlowAt(sender, flowRate, block.timestamp);
        return
            vault.afterAgreementCreated(
                superToken,
                address(cfa),
                bytes32(0),
                abi.encode(sender, address(vault)),
    ...
    function updateFlow(
        ...
        bytes memory cbdata = vault.beforeAgreementUpdated(
            superToken,
            address(cfa),
            bytes32(0),
            abi.encode(sender, address(vault)),
    ...
    function terminateFlow(
        ...
        bytes memory cbdata = vault.beforeAgreementTerminated(
            superToken,
            address(cfa),
            bytes32(0),
            abi.encode(sender, address(vault)),
        ...
        return
            vault.afterAgreementTerminated(
                superToken,
                address(cfa),
                bytes32(0),
                abi.encode(sender, address(vault)),
...
contract CeloGdAntSeedVaultTest {
    ...
    function setUp() public {
        ...
        ERC1967Proxy proxy = new ERC1967Proxy(
            ...
            abi.encodeCall(
                ...
                (address(this), address(host), address(cfa))
            )
        );
    ...
    function testClassicDepositTransfersGd() public {
        ...
        require(
            vault.totalDepositedGd(address(user)) == 100 ether,
            "deposit recorded"
        );
    ...
    function testErc677TransferAndCallSingleTxDeposit() public {
        ...
        require(
            vault.totalDepositedGd(address(user)) == 25 ether,
            "erc677 deposit recorded"
        );
    ...
    function testErc777TokensReceivedSingleTxDeposit() public {
        ...
        require(
            vault.totalDepositedGd(address(user)) == 30 ether,
            "erc777 deposit recorded"
        );
        require(token.balanceOf(address(vault)) == 30 ether, "vault funded");
    }

    function testRejectsFirstDepositBelowOneUsdAndAllowsLaterSmallTopUps()
        public
    {
        setUp();
        user.approveVault(2 ether);
        (bool ok, ) = address(user).call(
            abi.encodeWithSignature("deposit(uint256)", 0.5 ether)
        );
        ...
        require(
            vault.totalDepositedGd(address(user)) == 1.25 ether,
            "subsequent top up allowed"
        );
    }
    ...
    function testSuperAppCreateUpdateTerminateStreamLifecycleAndPreservesCtx()
        public
    {
        setUp();
        ...
        bytes memory returnedCtx = host.createFlow(
            vault,
            address(token),
            address(user),
            flowRate,
        ...
        require(
            keccak256(returnedCtx) == keccak256(ctx),
            "update ctx preserved"
        );
        require(
            vault.streamBuyer(address(user)) == BUYER,
            "buyer preserved on update"
        );

        returnedCtx = host.terminateFlow(
            vault,
            address(token),
            address(user),
            ctx
        ...
        require(
            keccak256(returnedCtx) == keccak256(ctx),
            "termination ctx preserved"
        );
    }

    function testSuperAppRejectsWrongTokenAgreementReceiverAndNegativeFlow()
        public
    {
        setUp();
    ...
    function testUsesStaticOracleForFirstDepositThreshold() public {
        ...
        user.approveVault(2 ether);
        (bool ok, ) = address(user).call(
            abi.encodeWithSignature("deposit(uint256)", 1.5 ether)
        );
        require(!ok, "first deposit below $1 reserve equivalent rejected");

        user.deposit(2 ether);
        require(
            vault.totalDepositedGd(address(user)) == 2 ether,
            "deposit at oracle-derived threshold succeeds"
        );
    }
    ...
    function testUsesStaticOracleForStreamMinimum() public {
        ...
        uint256 minFlowValue = (uint256(5 ether) + monthSeconds - 1) /
            monthSeconds;
        ...
        require(
            keccak256(returnedCtx) == keccak256(buyerCtx),
            "ctx passthrough"
        );
    }

    function testAcceptsHighStaticOraclePriceWithoutReasonableBoundsFallback()
        public
    {
        setUp();
        MockStaticOracle oracle = new MockStaticOracle();
        ...
        require(
            vault.totalDepositedGd(address(user)) == 0.001 ether,
            "high reserve price should be used directly"
        );
    }
    ...
    function testErc777RejectsWrongReceiverInCallback() public {
        setUp();
        bool ok = token.callTokensReceivedWithTo(
            address(vault),
            address(user),
            address(0xBEEF),
            1 ether,
    ...
    function testTokenFallbackDeposit() public {
        ...
        bool ok = token.callTokenFallback(
            address(vault),
            address(this),
            50 ether,
            abi.encode(BUYER)
        ...
        require(
            vault.totalDepositedGd(address(this)) == 50 ether,
            "tokenFallback deposit recorded"
        );
    }
    ...
    function testTransferFailedRevertsDeposit() public {
        ...
        CeloGdAntSeedVault failImpl = new CeloGdAntSeedVault(
            address(failToken)
        );
    ...
    function testSetStaticOracleConfigRejectsInvalidPriceConfig() public {
        ...
        (bool ok, ) = address(vault).call(
            abi.encodeWithSignature(
                "setStaticOracleConfig(address,address,uint256)",
                address(0),
                address(0),
                uint256(0)
            )
    ...
    function testOnlyOwnerFunctionsRejectNonOwner() public {
        ...
        (ok, ) = address(outsider).call(
            abi.encodeWithSignature(
                "setSuperfluidConfig(address,address)",
                address(0),
                address(0)
            )
        );
        ...
        (ok, ) = address(outsider).call(
            abi.encodeWithSignature(
                "setMinimumUsdThresholds(uint256,uint256)",
                uint256(1),
                uint256(1)
            )
        );
    ...
    function testSetMinimumUsdThresholds() public {
        ...
        require(
            vault.minFirstDepositUsd() == 2_000_000,
            "min deposit threshold updated"
        );
        require(
            vault.minMonthlyStreamUsd() == 3_000_000,
            "min stream threshold updated"
        );
    }
    ...
    function testGdUsdPerTokenReturnsFallbackWhenOracleNotSet() public {
        ...
        require(
            vault.gdUsdPerToken(1e18) == vault.fallbackGdUsdPerToken(),
            "returns fallback when no oracle"
        );
    }
    ...
    function testGdUsdPerTokenReturnsOraclePrice() public {
        ...
        vault.setStaticOracleConfig(address(oracle), address(0xCCCC), 0);
        require(
            vault.gdUsdPerToken(1e18) == 5e17,
            "returns oracle quote directly"
        );
    }
    ...
    function testGdUsdPerTokenFallsBackWhenOracleReturnsZero() public {
        ...
        vault.setStaticOracleConfig(
            address(oracle),
            address(0xCCCC),
            1_000_000
        );
        require(
            vault.gdUsdPerToken(1e18) ==
                (uint256(1e18) * 1e18) / vault.fallbackGdUsdPerToken(),
            "falls back when oracle returns zero"
        );
    }
    ...
    function testGdUsdPerTokenFallsBackWhenOracleReverts() public {
        ...
        require(
            vault.gdUsdPerToken(1e18) ==
                (uint256(1e18) * 1e18) / vault.fallbackGdUsdPerToken(),
            "falls back when oracle reverts"
        );
    }
